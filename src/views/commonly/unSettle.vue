    ...
    <table class="ex-table">
      ...
      <tfoot>
        <tr class="total-tr">
          ...
          <td></td>
          <td class="b-c-text">{{pageSum.Result}}</td>
        </tr>
        <tr class="total-tr" v-show="pageSum.Tot > pageConfig.limit">
          ...
          <td>{{allSum.Amount}}</td>
          <td></td>
          <td class="b-c-text">{{allSum.Result}}</td>
        </tr>
      </tfoot>
    </table>
...
<script>
import {iGetDuplexDetail, iGetUnsettlOrder} from "@/scripts/api/commonly";
export default {
  //未结明细
  name: "unSettle",
  components:{
  ...
  methods:{
    ...
    async getUnsettlOrder(){
      this.loading = true
      try {
        let res = await iGetUnsettlOrder(this.pageConfig);
        this.tableData = res.BetList
        if(Object.keys(res.Sum).length > 0){
